...
- Moved all files from `Crystal_ChatboxV1.0/` subdirectory to root
- Updated `main.py` to run on `0.0.0.0:5000` (Replit compatible)
- Removed hardcoded credentials from `settings.json` for security
- Added `.gitignore` for Python projects
- Configured workflow for Flask application
- Configured deployment with Gunicorn for production
- **Added PyWebview GUI support** for desktop applications
- **Added A-Ads integration** (hidden for Patreon supporters)
- **Fixed Spotify Windows compatibility** (use 127.0.0.1 instead of localhost)
- **Fixed download settings** for Windows
- **Added Android support** via buildozer.spec
- Changed "Quest IP" to "Quest or Desktop IP" throughout
