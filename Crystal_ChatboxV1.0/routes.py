...
def get_current_preview():
    ...
    window_line = ""
    print(f"[DEBUG] show_window={show_window}, app_name={wstate.get('app_name')}")
    if show_window and wstate.get("app_name"):
        ...
        print(f"[DEBUG] window_line created: {window_line}")
    ...
    print(f"[DEBUG] Layout order: {layout}")
    for part in layout:
        ...
        if part == "time" and current_time_text:
            ...
            print(f"[DEBUG] show_custom={show_custom}, current_custom_text={current_custom_text}")
            ...
            print(f"[DEBUG] MATCH! Adding window_line to lines: {window_line}")
            lines.append(window_line)
        elif part == "window":
            print(f"[DEBUG] window part found but window_line is empty/false: {repr(window_line)}")
        elif part == "heartrate" and heartrate_line:
            lines.append(heartrate_line)

    result = "\n".join(lines).strip()
    print(f"[DEBUG] Final lines array: {lines}")
    print(f"[DEBUG] Final message result: {repr(result)}")
    return result
        ...
        while True:
            try:
                ...
                if current_quest_ip != last_quest_ip:
                    print(f"[Auto-Reconnect] Quest IP changed from {last_quest_ip} to {current_quest_ip}")
                    client = make_client()
...
def create_app():
    ...
    def download_settings():
        return send_file(SETTINGS_FILE, as_attachment=True, download_name=f"vrchat_chatbox_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
